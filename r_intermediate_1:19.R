...
#can change print to do something else too
for (i in 1:11) {
  print(mean(class(dat[ , i])))
}

#### "i" can be anything, it constantly changes value 
#' 
...
#+ eval = FALSE
for (i in 1:11) { 
  print(mean(dat[ ,i])) # note this line is 4 spaces from the left margin, 2 spaces is also common, 0 spaces is bad form
}
#' 
...
#here is some practice: changing some stuff around
dat_means <- NULL
for (i in 1:ncol(dat)) {
  dat_means[i] <- mean(dat[ , i], na.rm = TRUE)
  if(dat_means[i] > 20) {
    print('sweet!')
  }
  else {
 ...
 ##Did not work, will figure out why^^^^
...
#' 
x <- 1:10
ifelse(x > 5 , 'sweet!', 'sour!'). #if using with a data frame, will do for every value in rows and columns
#' 
...
#my practice: calculate mean of every column
eval_mean <- function(x, na.rm=TRUE) {
  dat_means <- NULL
  for (i in 1:ncol(x)) {
    dat_means[i] <- mean(x[ , i], na.rm=TRUE)
    if (dat_means[i] < 5) {
      print('less than 5')
    }
    else if (dat_means[i] == 5) {
      print('equal to 5')
    }
    else {
...
#calc just means, check video to fix syntax errors with this
eval_mean <- function(x, na.rm=TRUE) {
  dat_means <- NULL
  for (i in 1:ncol(x)) {
    dat_means[i] <- mean(x[ , i], na.rm=TRUE)
